...
const extension_1 = require("../../extension"); // Import actual regexes
...
function lineAt(text, offsetOrLine) {
    let lineContent;
    let lineNum;
    let lineStartOffset;
    let lineEndOffset;
    let nextLineStartOffset = undefined;
...
function getTestEdits(documentText) {
    let edits = [];
    const combinedRegex = new RegExp(`(${extension_1.PRINT_LOG_REGEX.source})|(${extension_1.LOG_FUNCTION_REGEX.source})`, 'gm' // Ensure 'g' flag is here, 'm' for multiline ^ $ matching
    );
    let match;
    ...
    suite('Regular Expression Tests', () => {
        test('PRINT_LOG_REGEX should match valid print statements', () => {
            ...
            validPrints.forEach(s => {
                extension_1.PRINT_LOG_REGEX.lastIndex = 0; // Reset regex state
                assert.ok(extension_1.PRINT_LOG_REGEX.test(s), `Failed for: ${s}`);
            });
        });
        test('PRINT_LOG_REGEX should not match invalid print statements', () => {
            ...
            invalidPrints.forEach(s => {
                extension_1.PRINT_LOG_REGEX.lastIndex = 0;
                assert.strictEqual(extension_1.PRINT_LOG_REGEX.test(s), false, `Incorrectly matched: ${s}`);
            });
        });
        test('LOG_FUNCTION_REGEX should match valid log statements', () => {
            ...
            validLogs.forEach(s => {
                extension_1.LOG_FUNCTION_REGEX.lastIndex = 0;
                assert.ok(extension_1.LOG_FUNCTION_REGEX.test(s), `Failed for: ${s}`);
            });
        });
        test('LOG_FUNCTION_REGEX should not match invalid log statements', () => {
            ...
            invalidLogs.forEach(s => {
                extension_1.LOG_FUNCTION_REGEX.lastIndex = 0;
                assert.strictEqual(extension_1.LOG_FUNCTION_REGEX.test(s), false, `Incorrectly matched: ${s}`);
            });
        });
    });
